...
# ══════════════════════════════════════════════════════════════
#  Fix for pythonw.exe: stdout/stderr are None without console.
#  Must be BEFORE any import that might print or write.
#  This fixes: 'NoneType' object has no attribute 'write'
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  tqdm monkey-patch: captures progress from whisper download
#  and transcription, reports to UI via callback.
# ══════════════════════════════════════════════════════════════
...
try:
    ...
    class _ProgressTqdmCls(_orig_tqdm):
        """tqdm subclass that forwards progress to a UI callback."""

        def __init__(self, *args, **kwargs):
            kwargs["file"] = _devnull_f
        ...
        def update(self, n=1):
            ...
            if cb and self.total and self.total > 0:
                now = time.time()
                if now - _progress_last_t < 0.12:
                    return
                _progress_last_t = now
                pct = min(100.0, (self.n / self.total) * 100)
                try:
                    cb(pct, self.n, self.total)
                except Exception:
                    pass

    _ProgressTqdm = _ProgressTqdmCls
...
class TranscritorApp:
    ...
    TAMANHO_MINIMO = {
        "tiny":    70_000_000,   "base":   130_000_000,
        "small":  440_000_000,   "medium":1_400_000_000,
        "large": 2_800_000_000,
    }
    ...
    TEXTOS = {
      "pt": {
        ...
        "status_carregando_cache":
            "Carregando modelo {modelo} do cache…",
        "status_carregando_n":
        ...
        "status_concluido":
            ...
            "  ·  {tempo}",
        "status_arquivos":      "{n} arquivo(s) selecionado(s)",
        "status_retry":
            "Download falhou. Tentando novamente em {seg}s…",
        "status_cache_limpo":
        ...
        "toast_concluido":
            "Transcrição concluída — {palavras} palavras em {tempo}",
        ...
        "modelo_desc": {
            "tiny": "Ultra rápido", "base": "Rápido",
            "small": "Equilibrado", "medium": "Alta qualidade",
            "large": "Máxima precisão",
        },
      },
      "en": {
        "window_title":         "Audio to Text — Ruan Almeida",
        "header_title":         "Audio to Text Transcription",
        "header_sub":           "Ruan Almeida  ·  2025  ·  @ruanalmeidar",
        "links_label":          "Links:",
        "drop_title":           "Click here to select audio",
        "drop_formats":
            "MP3 · WAV · M4A · OGG · FLAC · AAC · WMA · MP4",
        ...
        "status_carregando_cache":
            "Loading model {modelo} from cache…",
        "status_carregando_n":
            "Downloading model {modelo}… attempt {t}/{max}",
        "status_carregado":     "Model {modelo} loaded!",
        "status_transcrevendo":
            "[{modelo}]  Transcribing ({i}/{n}):  {arq}",
        "status_concluido":
            "Done!  {modelo}  ·  {palavras} words  ·  {tempo}",
        "status_arquivos":      "{n} file(s) selected",
        "status_retry":
            "Download failed. Retrying in {seg}s…",
        "status_cache_limpo":
            "Partial cache removed. Re-downloading {modelo}…",
        "toast_copiado":        "Copied to clipboard!",
        ...
        "toast_concluido":
            "Transcription complete — {palavras} words in {tempo}",
        "toast_arquivos":       "{n} file(s) loaded",
        "msg_aguarde_titulo":   "Wait",
        "msg_aguarde":          "Transcription in progress.",
        "msg_aviso":            "Warning",
        "msg_selecione":        "Select an audio first.",
        ...
        "modelo_desc": {
            "tiny": "Ultra fast", "base": "Fast",
            "small": "Balanced", "medium": "High quality",
            "large": "Max accuracy",
        },
      },
    }
    ...
    def __init__(self, root):
        ...
        # progress state
        self._progress_ctx    = None   # "download" | "transcribe"
        self._file_idx        = 0
        self._file_total      = 0
    ...
    def _aplicar_estilo_ttk(self):
        ...
        s.configure("D.Horizontal.TProgressbar",
                     ...
                     darkcolor=self.PINK, thickness=5)

    # ===================== HELPERS ==================
    def _t(self, k):
        return self.TEXTOS[self.ui_lang].get(k, f"[{k}]")

    def _get_whisper_code(self):
        n = self.var_idioma.get()
    ...
    def _criar_widgets(self):
        # ── HEADER ──
        hdr = tk.Frame(self.root, bg=self.BG_DARKER)
        hdr.pack(fill="x")
        hi = tk.Frame(hdr, bg=self.BG_DARKER)
        hi.pack(pady=(18, 14))
        self.lbl_title = tk.Label(hi,
            text=self._t("header_title"),
            font=("Segoe UI", 20, "bold"),
            bg=self.BG_DARKER, fg=self.PURPLE)
        self.lbl_title.pack()
        self.lbl_sub = tk.Label(hi,
            text=self._t("header_sub"),
            font=("Segoe UI", 9),
            bg=self.BG_DARKER, fg=self.COMMENT)
        self.lbl_sub.pack(pady=(4, 0))
        tk.Frame(self.root, bg=self.CURRENT, height=1).pack(fill="x")

        # ── LINKS + FLAGS ──
        bar = tk.Frame(self.root, bg=self.BG)
        bar.pack(fill="x")
        bi = tk.Frame(bar, bg=self.BG)
        bi.pack(fill="x", padx=28, pady=8)
        lf = tk.Frame(bi, bg=self.BG); lf.pack(side="left")
        self.lbl_links = tk.Label(lf, text=self._t("links_label"),
            font=("Segoe UI", 9), bg=self.BG, fg=self.COMMENT)
        ...
        ff = tk.Frame(bi, bg=self.BG); ff.pack(side="right")
        self._criar_bandeiras(ff)
        tk.Frame(self.root, bg=self.CURRENT, height=1).pack(fill="x")
        ...
        self.drop_canvas = tk.Canvas(self.drop_frame, height=90,
            highlightthickness=0, bg=self.BG_DARKER, cursor="hand2")
        self.drop_canvas.pack(fill="both", expand=True, padx=1, pady=1)
        self.drop_canvas.bind("<Configure>", self._desenhar_drop)
        self.drop_canvas.bind("<Button-1>",
                              lambda e: self._selecionar_arquivos())
        self.drop_canvas.bind("<Enter>", self._drop_enter)
        self.drop_canvas.bind("<Leave>", self._drop_leave)

        # options row
        fo = tk.Frame(corpo, bg=self.BG); fo.pack(fill="x", pady=(0, 12))
        self.lbl_idioma_text = tk.Label(fo, text=self._t("lbl_idioma"),
            font=("Segoe UI", 10, "bold"), bg=self.BG, fg=self.FOREGROUND)
        ...
        self.cb_quebra = tk.Checkbutton(fo,
            text=self._t("chk_quebra"), variable=self.var_quebra_linha,
            font=("Segoe UI", 10), bg=self.BG, fg=self.FOREGROUND,
            selectcolor=self.BG_DARKER, activebackground=self.BG,
        ...
        self.cb_timestamps = tk.Checkbutton(fo,
            text=self._t("chk_timestamps"), variable=self.var_timestamps,
            font=("Segoe UI", 10), bg=self.BG, fg=self.FOREGROUND,
            selectcolor=self.BG_DARKER, activebackground=self.BG,
            activeforeground=self.FOREGROUND, bd=0,
            highlightthickness=0, cursor="hand2")
        ...
        tk.Label(fo, text=dev, font=("Segoe UI", 9, "bold"),
                 bg=self.BG, fg=dc).pack(side="left")
        rf = tk.Frame(fo, bg=self.BG); rf.pack(side="right")
        self.lbl_fonte = tk.Label(rf, text=self._t("font_label"),
            font=("Segoe UI", 9), bg=self.BG, fg=self.COMMENT)
        self.lbl_fonte.pack(side="left", padx=(0, 4))
        for txt, cmd in [(" A+ ", self._font_mais),
                         (" A- ", self._font_menos)]:
            b = tk.Button(rf, text=txt, font=("Segoe UI", 9, "bold"),
                bg=self.CURRENT, fg=self.FOREGROUND, relief="flat",
                bd=0, cursor="hand2",
                activebackground=self.BG_LIGHTER,
                activeforeground=self.FOREGROUND, command=cmd)
        ...
        for i, m in enumerate(self.MODELOS):
            ...
            btn = tk.Button(inner,
                ...
                command=lambda k=m["key"]: self._clique_modelo(k))
            btn.pack(fill="both", expand=True)
            self.btns_modelos[m["key"]] = {
                "btn": btn, "cor": m["cor"], "outer": outer}
            cor_m = m["cor"]
            def _me(e, b=btn, c=cor_m, o=outer):
                if str(b.cget("state")) != "disabled":
                    b.config(bg=self.BG_LIGHTER); o.config(bg=c)
            def _ml(e, b=btn, o=outer):
                if str(b.cget("state")) != "disabled":
                    b.config(bg=self.BG_DARKER); o.config(bg=self.CURRENT)
            btn.bind("<Enter>", _me); btn.bind("<Leave>", _ml)
        ...
        self.lbl_progress_pct = tk.Label(fp, text="",
            font=("Consolas", 9, "bold"),
            bg=self.BG, fg=self.PURPLE,
            width=24, anchor="center")
        self.lbl_progress_pct.pack(side="left", padx=(0, 8))
        ...
        self.label_info.pack(side="right", fill="x", expand=True)

        # text area
        self.text_border = tk.Frame(corpo, bg=self.CURRENT, bd=0)
        self.text_border.pack(fill="both", expand=True)
        ti = tk.Frame(self.text_border, bg=self.BG_DARKER, bd=0)
        ti.pack(fill="both", expand=True, padx=1, pady=1)
        self.texto_resultado = scrolledtext.ScrolledText(ti,
            wrap="word", font=("Consolas", self.font_size),
        ...
        # footer
        tk.Frame(self.root, bg=self.CURRENT, height=1).pack(
            fill="x", side="bottom")
        ft = tk.Frame(self.root, bg=self.BG_DARKER, height=30)
        ft.pack(fill="x", side="bottom"); ft.pack_propagate(False)
        self.lbl_footer = tk.Label(ft, text=self._t("footer"),
            font=("Segoe UI", 8), bg=self.BG_DARKER, fg=self.COMMENT)
    ...
    def _criar_bandeiras(self, parent):
        self.flag_br_frame = tk.Frame(parent, bg=self.PURPLE,
                                      cursor="hand2")
        self.flag_br_frame.pack(side="left", padx=(0, 5))
        self.flag_br = tk.Canvas(self.flag_br_frame, width=38,
            height=26, highlightthickness=0, cursor="hand2")
        ...
        self.flag_br_frame.bind("<Button-1>",
            lambda e: self._trocar_idioma_ui("pt"))
        self.flag_br.bind("<Enter>", lambda e:
            self.flag_br_frame.config(
                bg=self.PURPLE if self.ui_lang == "pt"
                else self.COMMENT))
        self.flag_br.bind("<Leave>", lambda e:
            self.flag_br_frame.config(
                bg=self.PURPLE if self.ui_lang == "pt"
                else self.CURRENT))

        self.flag_us_frame = tk.Frame(parent, bg=self.CURRENT,
                                      cursor="hand2")
        self.flag_us_frame.pack(side="left")
        self.flag_us = tk.Canvas(self.flag_us_frame, width=38,
            height=26, highlightthickness=0, cursor="hand2")
        ...
        self.flag_us_frame.bind("<Button-1>",
            lambda e: self._trocar_idioma_ui("en"))
        self.flag_us.bind("<Enter>", lambda e:
            self.flag_us_frame.config(
                bg=self.PURPLE if self.ui_lang == "en"
                else self.COMMENT))
        self.flag_us.bind("<Leave>", lambda e:
            self.flag_us_frame.config(
                bg=self.PURPLE if self.ui_lang == "en"
                else self.CURRENT))
    ...
    def _draw_flag_br(self):
        ...
        c.create_rectangle(0, 0, 38, 26, fill="#009c3b", outline="")
        c.create_polygon(19, 2, 36, 13, 19, 24, 2, 13,
                         fill="#ffdf00", outline="")
        c.create_oval(11, 7, 27, 20, fill="#002776", outline="")
    ...
    def _draw_flag_us(self):
        ...
        for i in range(13):
            ...
            y1, y2 = round(i * 26 / 13), round((i + 1) * 26 / 13)
            c.create_rectangle(0, y1, 38, y2, fill=cor, outline="")
        c.create_rectangle(0, 0, 16, 14, fill="#3C3B6E", outline="")
        for row, cols in enumerate([(3, 8, 13), (5, 11), (3, 8, 13)]):
            for cx in cols:
                c.create_text(cx, 3 + row * 4, text="·",
                    fill="white", font=("Segoe UI", 5, "bold"))

    # ===================== LANGUAGE =================
    def _trocar_idioma_ui(self, nl):
        if nl == self.ui_lang: return
        on = self.var_idioma.get()
        sc = "pt"
        for l in self.WHISPER_LANGS:
            if l[self.ui_lang] == on: sc = l["code"]; break
        self.ui_lang = nl
        for l in self.WHISPER_LANGS:
            if l["code"] == sc: self.var_idioma.set(l[nl]); break
        if nl == "pt":
        ...
        for m in self.MODELOS:
            ...
            self.btns_modelos[k]["btn"].config(
                text=f"{m['nome']}\n{m['tam']}\n"
                     f"{'★'*m['qual']}{'☆'*(5-m['qual'])}\n"
                     f"{descs[k]}")
        self.btn_copiar.config(text=f"  {t('btn_copiar')}  ")
        self.btn_salvar_txt.config(text=f"  {t('btn_salvar_txt')}  ")
        self.btn_salvar_srt.config(text=f"  {t('btn_salvar_srt')}  ")
        self.btn_limpar.config(text=f"  {t('btn_limpar')}  ")
        if not self.transcrevendo:
            self.label_status.config(text=t("status_pronto"))
        self.lbl_footer.config(text=t("footer"))
        self._rebuild_menu_idioma()

    # ===================== IDIOMA MENU ==============
    def _criar_menu_idioma(self, parent):
        nomes = [l[self.ui_lang] for l in self.WHISPER_LANGS]
        self.menu_idioma = tk.OptionMenu(parent, self.var_idioma, *nomes)
        self.menu_idioma.config(bg=self.CURRENT, fg=self.CYAN,
            font=("Segoe UI", 10, "bold"), relief="flat",
            highlightthickness=0, bd=0, padx=10, pady=2,
            activebackground=self.BG_LIGHTER,
            activeforeground=self.CYAN)
        self.menu_idioma["menu"].config(bg=self.BG_DARKER,
            fg=self.FOREGROUND, font=("Segoe UI", 10),
            activebackground=self.PURPLE,
            activeforeground=self.FOREGROUND, bd=0)
        self.menu_idioma.pack(side="left", padx=(6, 14))

    def _rebuild_menu_idioma(self):
        menu = self.menu_idioma["menu"]; menu.delete(0, "end")
        for info in self.WHISPER_LANGS:
            n = info[self.ui_lang]
            menu.add_command(label=n,
                command=lambda v=n: self.var_idioma.set(v))
        menu.config(bg=self.BG_DARKER, fg=self.FOREGROUND,
            font=("Segoe UI", 10), activebackground=self.PURPLE,
            activeforeground=self.FOREGROUND, bd=0)

    # ===================== SMALL HELPERS ============
    def _sep_v(self, p):
        tk.Frame(p, bg=self.CURRENT, width=1).pack(
            side="left", fill="y", padx=(0, 12), pady=4)

    def _hover_btn(self, w, n, h):
        w.bind("<Enter>", lambda e: w.config(bg=h)
               if str(w.cget("state")) != "disabled" else None)
        w.bind("<Leave>", lambda e: w.config(bg=n)
               if str(w.cget("state")) != "disabled" else None)

    def _action_btn(self, p, txt, cmd, cor, px):
        b = tk.Button(p, text=f"  {txt}  ",
            font=("Segoe UI", 10, "bold"),
            bg=self.CURRENT, fg=cor, relief="flat",
            pady=5, padx=6, bd=0, cursor="hand2",
            activebackground=self.BG_LIGHTER,
    ...
    def _tamanho_arquivo(path):
        try:
            ...
            if sz < 1024**2:    return f"{sz/1024:.0f} KB"
    ...
    def _status(self, msg, cor=None):
        self.label_status.config(text=msg, fg=cor or self.FOREGROUND)
        self.root.update_idletasks()

    def _toast(self, msg, cor=None, dur=2500):
        cor = cor or self.GREEN
        self.toast_label.config(text=f"  {msg}  ", fg=cor,
                                bg=self.BG_DARKER)
        self.toast_label.place(relx=0.5, y=95, anchor="center")
        if self._toast_id:
            self.root.after_cancel(self._toast_id)
        self._toast_id = self.root.after(
            dur, lambda: self.toast_label.place_forget())
    ...
    def _font_mais(self):
        ...
        self.texto_resultado.config(font=("Consolas", self.font_size))
    ...
    # ===================== PROGRESS =================
    def _on_tqdm_progress(self, pct, n, total):
        """Called from tqdm patch on worker thread."""
        ctx = self._progress_ctx
        if ctx == "download":
            done = self._format_size(n)
            tot  = self._format_size(total)
            info = f"{pct:.0f}%  ({done} / {tot})"
            self.root.after(0, self._update_progress, pct, info)
        elif ctx == "transcribe":
            if self._file_total > 1:
                overall = (
                    (self._file_idx - 1) / self._file_total * 100
                    + pct / self._file_total)
            else:
                overall = pct
    ...
    def _update_progress(self, pct, info):
        ...
        try:
            ...
            self.lbl_progress_pct.config(text=info)
        except Exception:
            pass
    ...
    def _reset_progress(self):
        ...
        try:
            ...
            self.progress.config(mode="determinate",
                                 maximum=100, value=0)
            self.lbl_progress_pct.config(text="")
        except Exception:
            pass
    ...
    def _desenhar_drop(self, event=None):
        ...
        if w < 20: return
        c.delete("all")
        borda = self.PURPLE if self._drop_hover else self.COMMENT
        c.create_rectangle(10, 8, w-10, h-8,
                           outline=borda, width=1, dash=(8, 4))
        if not self.arquivos:
            c.create_text(w//2, h//2-14, text=self._t("drop_title"),
                font=("Segoe UI", 13, "bold"), fill=self.PURPLE)
            c.create_text(w//2, h//2+8, text=self._t("drop_formats"),
                font=("Segoe UI", 9), fill=self.COMMENT)
            c.create_text(w//2, h//2+26, text=self._t("drop_hint"),
                font=("Segoe UI", 8), fill=self.CURRENT)
        else:
            nomes = [os.path.basename(f) for f in self.arquivos]
            tams  = [self._tamanho_arquivo(f) for f in self.arquivos]
            info  = "  |  ".join(
                f"{n} ({t})" for n, t in zip(nomes, tams))
            if len(info) > 85: info = info[:82] + "…"
            c.create_text(w//2, h//2-10, text=info,
                font=("Segoe UI", 11, "bold"), fill=self.GREEN)
            c.create_text(w//2, h//2+12,
                text=(self._t("drop_files").format(n=len(self.arquivos))
                      + "  ·  " + self._t("drop_change")),
                font=("Segoe UI", 9), fill=self.COMMENT)
    ...
    def _limpar_tudo(self):
        ...
        self.label_timer.config(text="00:00", fg=self.COMMENT)
        self._reset_progress()
        self._status(self._t("status_pronto_curto"), self.GREEN)
        self._toast(self._t("toast_limpo"), self.ORANGE)

    # ===================== COPIAR / SALVAR ==========
    def _copiar_texto(self):
        txt = self.texto_resultado.get("1.0", tk.END).strip()
        if txt:
            self.root.clipboard_clear()
            self.root.clipboard_append(txt)
            self._status(self._t("status_copiado"), self.GREEN)
            self._toast(self._t("toast_copiado"), self.GREEN)
        else:
            self._status(self._t("status_nada_copiar"), self.ORANGE)
            self._toast(self._t("toast_nada_copiar"), self.ORANGE)

    def _salvar_txt(self):
        txt = self.texto_resultado.get("1.0", tk.END).strip()
        if not txt:
            messagebox.showwarning(
                self._t("msg_aviso"), self._t("msg_sem_texto"))
            return
        cam = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[(self._t("dlg_tipo_texto"), "*.txt"),
                       (self._t("dlg_tipo_todos"), "*.*")],
        ...
        if cam:
            with open(cam, "w", encoding="utf-8") as f:
                f.write(txt)
            self._status(
                self._t("status_txt_salvo").format(cam=cam),
                self.GREEN)
            self._toast(self._t("toast_txt_salvo"), self.GREEN)

    def _salvar_srt(self):
        if not self.segmentos_resultado:
            messagebox.showwarning(
                self._t("msg_aviso"), self._t("msg_sem_seg"))
            return
        cam = filedialog.asksaveasfilename(
            defaultextension=".srt",
            filetypes=[(self._t("dlg_tipo_srt"), "*.srt"),
                       (self._t("dlg_tipo_todos"), "*.*")],
            ...
            with open(cam, "w", encoding="utf-8") as f:
                for i, seg in enumerate(
                    ...
                    f.write(f"{i}\n{ini} --> {fim}\n"
                            f"{seg['text'].strip()}\n\n")
            self._status(
                self._t("status_srt_salvo").format(cam=cam),
                self.GREEN)
            self._toast(self._t("toast_srt_salvo"), self.GREEN)
    ...
    def _limpar_cache_modelo(self, nome_modelo):
        ...
        if not os.path.isdir(cache_dir):
            return
        for pattern in [f"{nome_modelo}*.pt",
                        f"{nome_modelo}*.download"]:
            for fp in glob.glob(
                    os.path.join(cache_dir, pattern)):
                try:
                    sz = os.path.getsize(fp)
                    mn = self.TAMANHO_MINIMO.get(nome_modelo, 0)
                    if sz < mn or fp.endswith(".download"):
                        os.remove(fp)
                except Exception:
                    pass
    ...
    def _modelo_ja_em_cache(self, nome_modelo):
        ...
        if not os.path.isdir(cache_dir):
            return False
        mn = self.TAMANHO_MINIMO.get(nome_modelo, 0)
        for pat in [f"{nome_modelo}.pt",
                    f"{nome_modelo}-v2.pt",
                    f"{nome_modelo}-v3.pt"]:
            for fp in glob.glob(
                    os.path.join(cache_dir, pat)):
                try:
                    if os.path.getsize(fp) >= mn:
                        return True
                except Exception:
                    pass
        return False
    ...
    def _carregar_modelo_com_retry(self, nome_modelo):
        ...
        # Patch whisper's tqdm reference so download
        # progress goes through our callback
        if _TQDM_PATCHED and _ProgressTqdm is not None:
            try:
                whisper.tqdm = _ProgressTqdm
            except Exception:
                pass

        max_t = self.MAX_RETRY
        ultimo_erro = None

        for tentativa in range(1, max_t + 1):
            try:
                em_cache = self._modelo_ja_em_cache(nome_modelo)

                if em_cache:
                    self._status(
                        self._t("status_carregando_cache").format(
                            modelo=nome_modelo.upper()),
                        self.ORANGE)
                    self.root.after(0, self._start_indeterminate)
                elif tentativa == 1:
                    self._status(
                        self._t("status_carregando").format(
                            modelo=nome_modelo.upper()),
                        self.ORANGE)
                    self.root.after(0, self._reset_progress)
                else:
                    self._status(
                        self._t("status_carregando_n").format(
                            modelo=nome_modelo.upper(),
                ...
                is_rede = any(kw in erro_str
                              for kw in erros_rede)

                if is_rede and tentativa < max_t:
                    self._limpar_cache_modelo(nome_modelo)
                    pausa = self.RETRY_WAIT[
                        min(tentativa - 1,
                            len(self.RETRY_WAIT) - 1)]
                    self._status(
                        self._t("status_retry").format(
                            seg=pausa), self.RED)
                    time.sleep(pausa)
                    self._status(
                        self._t("status_cache_limpo").format(
                            modelo=nome_modelo.upper()),
                        self.ORANGE)
                    time.sleep(1)
                else:
    ...
    def _clique_modelo(self, nome_modelo):
        ...
        for nm, info in self.btns_modelos.items():
            if nm == nome_modelo:
                info["btn"].config(bg=info["cor"],
                                   fg=self.BG_DARKER)
                info["outer"].config(bg=info["cor"])
            else:
                info["btn"].config(bg=self.BG_DARKER,
                                   fg=info["cor"])
    ...
    def _transcrever(self, nome_modelo):
        ...
        try:
            ...
            # ── Transcription phase ──
            self._progress_ctx = "transcribe"
            self._file_total = len(self.arquivos)

            # Reset progress for transcription
            self.root.after(0, self._reset_progress)
            time.sleep(0.1)
            ...
            for i, arq in enumerate(self.arquivos, 1):
                ...
                self._status(
                    self._t("status_transcrevendo").format(
                        modelo=nome_modelo.upper(),
                        i=i, n=self._file_total,
                        arq=nome_arq),
                    self.PURPLE)
                ...
                idioma_det = res.get("language", idioma_det)

                # Update progress for completed file
                if self._file_total > 1:
                    file_pct = (i / self._file_total) * 100
                    self.root.after(0, self._update_progress,
                                    file_pct,
                                    f"{file_pct:.0f}%")
                ...
                if usar_ts:
                    ...
                    if quebrar:
                        texto_bruto = \
                            self._formatar_texto(texto_bruto)
    ...
    def _resultado_erro(self, msg):
        def up():
            ...
            self._habilitar()
            self.transcrevendo = False
            self._status(self._t("status_erro"), self.RED)
            self._toast(self._t("toast_erro"), self.RED)
